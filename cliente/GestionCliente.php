...
if (isset($delete_success) && $delete_success) {
    echo "<script>Swal.fire('Eliminado!', 'El cliente ha sido eliminado.', 'success');</script>";
    ...
    <ul class="navbar-nav ms-auto ms-md-0 me-3 me-lg-4">
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4 py-4">
                        <!-- Sección de Registro de Cliente (solo admin) -->
                        <?php if ($rol === 'admin'): ?>
                                    ...
                                    <div class="card-body">
                                        <form action="../controlador/registroCliente.php" method="POST">
                                            <div class="form-floating mb-3">
                                                <input class="form-control" id="inputNombre" name="nombre" type="text" placeholder="Nombre completo" required />
                                                <label for="inputNombre">Nombre completo</label>
                                            </div>
                                            <div class="form-floating mb-3">
                                                <input class="form-control" id="inputTelefono" name="telefono" type="tel" placeholder="Teléfono" />
                                                <label for="inputTelefono">Teléfono</label>
                                            </div>
                                            <div class="form-floating mb-3">
                                                <input class="form-control" id="inputRut" name="rut" type="text" placeholder="RUT chileno" />
                                                <label for="inputRut">RUT chileno</label>
                                            </div>
                                            <div class="form-floating mb-3">
                                                <input class="form-control" id="inputCorreo" name="correo" type="email" placeholder="Correo electrónico" />
                                                <label for="inputCorreo">Correo electrónico</label>
                                            </div>
                                            <div class="form-floating mb-3">
                                                <input class="form-control" id="inputDireccion" name="direccion" type="text" placeholder="Dirección" />
                                            ...
                                            <div class="d-flex align-items-center justify-content-between mt-4 mb-0">
                                                <input class="btn btn-primary" type="submit" value="Registrar Cliente" name="gt">
                                            </div>
                                        </form>
                                    </div>
                                    <div class="card-footer text-center py-3">
                                        <div class="small"><a href="#clientes-section">Ver lista de clientes</a></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row justify-content-center mb-5">
                            <div class="col-lg-6">
                                <h4 class="text-center mb-4">Vista de Clientes (Solo Lectura)</h4>
                                <p class="text-center">Como vendedor, solo puedes consultar los clientes. Contacta a un administrador para modificaciones.</p>
                            </div>
                        </div>
                        <?php endif; ?>

                        <!-- Sección de Lista de Clientes -->
                        <div id="clientes-section" class="row justify-content-center">
                            <div class="col-lg-12">
                                <h4 class="text-center mb-4">Lista de Clientes Registrados</h4>
                                <div class="card shadow-lg border-0 rounded-lg">
                                    <div class="card-body">
                                        <div class="input-group mb-3">
                                            <input type="text" id="searchInput" class="form-control" placeholder="Buscar por nombre, correo o RUT...">
                                            ...
                                            <table class="table table-striped table-hover" id="clientesTable">
                                                ...
                                                <tbody>
                                                    ...
                                                    try {
                                                        ...
                                                        $stmt = $pdo->query("SELECT id_cliente, nombre, telefono, rut, correo, direccion, fecha_registro, total_gastado, ultima_compra 
                                                        ...
                                                        if ($stmt->rowCount() > 0) {
                                                            ...
                                                            while ($row = $stmt->fetch()) {
                                                                // Corregido: operador ternario completo para PHP 5.x
                                                                $ultima_compra = $row['ultima_compra'] ? $row['ultima_compra'] : 'N/A';
                                                                ...
                                                                echo "<tr>
                                                                        ...
                                                                        <td>\${$row['total_gastado']}</td>
                                                                        <td>{$ultima_compra}</td>
                                                                        <td>";
                                                                if ($rol === 'admin') {
                                                                    echo "<button class='btn btn-warning btn-sm me-2' onclick='updateClient({$row['id_cliente']})'>Actualizar</button>
                                                                          <button class='btn btn-danger btn-sm' onclick='deleteClient({$row['id_cliente']})'>Eliminar</button>";
                                                                } else {
                                                        ...
                                                        // Manejo de errores
                                                        echo "<tr><td colspan='10' class='text-center text-danger'>Error al cargar datos: " . $e->getMessage() . "</td></tr>";
                                                    }
                                                    ?>
                                                </tbody>
                                            </table>
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', () => {
                new simpleDatatables.DataTable("#clientesTable");
            });
            ...
            function filterTable() {
                ...
                for (let i = 1; i < rows.length; i++) { // Empieza desde 1 para saltar el header
                    const cells = rows[i].getElementsByTagName('td');
                    let match = false;
                    for (let j = 1; j < cells.length - 1; j++) { // Evita la columna de acciones
            ...
            function updateClient(id) {
                Swal.fire({
                    ...
                    text: '¿Estás seguro de que quieres actualizar este cliente?',
                    icon: 'question',
            ...
            function deleteClient(id) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        window.location.href = 'GestionCliente.php?delete_id=' + id;
                    }
                });
            }
        </script>
